...
export default function TestSimulationApp() {
  ...
  const [userHistory, setUserHistory] = useState(MockHistory);
  ...
  const getSoal = async () => {
    try {
      const res = await axios.get(process.env.NEXT_PUBLIC_API_URL + "/api/preservice/soal/premium/" + selectedPaket?._id)
      if (res.data) {
        setQuestions
  ...
  const handleSubmitTest = async (autoSubmit = false) => {
    ...
    try {
      const res = await axios.post(process.env.NEXT_PUBLIC_API_URL + "/api/preservice/nilai", resultData)
      if (res.data) {
