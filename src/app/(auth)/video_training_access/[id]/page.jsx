...
export default function DetailVideoTrainingAccess() {
  const { id } = useParams()
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50 py-16">
        <div className="max-w-5xl mx-auto px-4 py-8">
          <button
            onClick={() => { router.push("/video_training_access") }}
          ...
          <div className="bg-white rounded-md shadow-lg overflow-hidden mb-6">
            ...
            <div className="p-6">
              ...
              {videoData?.status == "BERBAYAR" && (
                <h1 className="text-xl font-bold text-gray-900 mb-4">
                  Rp{videoData?.harga?.toLocaleString('id-ID')}
                </h1>)
              }
              {
                videoData?.status == "BERBAYAR" && access?.status == "ACCESS" ? "" : <button
                  onClick={() => { router.push('/video_training/access?videoId=' + videoData._id + "&userId=" + user?._id + "&judul=" + videoData.judul+"&harga="+videoData?.harga) }}
          ...
          {videoData?.tujuanPembelajaran && (
            <div className="bg-white rounded-md shadow-lg p-6 mb-6">
              ...
              >
                {formatMultilineString(videoData?.tujuanPembelajaran)}
              </pre>
            </div>
          )}
          ...
          {videoData?.deskripsi && (
            <div className="bg-white rounded-md shadow-lg p-6">
              ...
              <pre
                ...
                className="text-gray-700 leading-relaxed"
              >
                {formatMultilineString(videoData?.deskripsi)}
              </pre>
            </div>
          )}
