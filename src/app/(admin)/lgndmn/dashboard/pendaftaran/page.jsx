...
export default function Pendaftaran() {
  ...
  const onDelete = async (id) => {
    try {
      const data = await axios.delete(process.env.NEXT_PUBLIC_API_URL + "/api/jadwal/" + id, { withCredentials: true });
      if (data.data) {
  ...
  const updateJadwal = async (id, status) => {
    try {
      const data = await axios.put(process.env.NEXT_PUBLIC_API_URL + "/api/jadwal/" + id, {
        status: status
      }, { withCredentials: true });

      if (data.data) {
        alert("Berhasil mengganti jadwal")
        setOnAdd(false)
      ...
      if (data.data) {
        ...
        for (let i = 0; i < data.data.length; i++) {
          if (data.data[i].hari == "Senin" && data.data[i].tanggal == "-") {
            ...
            newData.push({ _id: data.data[i]._id, hari: "Senin", tanggal: arrayTanggal[1], bulan: arrayTanggal[2], tahun: arrayTanggal[3], status: data.data[i].status })
          } else if (data.data[i].hari == "Rabu" && data.data[i].tanggal == "-") {
            let nextRabu = getNextDay("Rabu")
            let arrayTanggal = nextRabu.split(" ")
            newData.push({ _id: data.data[i]._id, hari: "Rabu", tanggal: arrayTanggal[1], bulan: arrayTanggal[2], tahun: arrayTanggal[3], status: data.data[i].status })
          } else if (data.data[i].hari == "Jumat" && data.data[i].tanggal == "-") {
            let nextJumat = getNextDay("Jumat")
            let arrayTanggal = nextJumat.split(" ")
            newData.push({ _id: data.data[i]._id, hari: "Jumat", tanggal: arrayTanggal[1], bulan: arrayTanggal[2], tahun: arrayTanggal[3], status: data.data[i].status })
          } else {
            newData.push({ _id: data.data[i]._id, hari: data.data[i].hari, tanggal: data.data[i].tanggal, bulan: data.data[i].bulan, tahun: data.data[i].tahun, status: data.data[i].status })
          }
  ...
  async function getListJadwal() {
    try {
      const data = await axios.get(process.env.NEXT_PUBLIC_API_URL + "/api/category", { withCredentials: true });
      if (data.data) {
    ...
    <div className="flex">
      ...
      <div className="w-[120%] mb-8">
        <div className="p-6 mt-8 text-center">
          <h1 className="text-3xl font-bold text-koreaBlue">TEACHER TEST REGISTRATION</h1>
        </div>
        <div className="m-auto w-full ">
          <div className=" m-auto bg-white p-3 rounded-lg w-full">
            <div className="bg-white px-6 pb-6 rounded-xl w-[1200px] max-w-3xl m-auto">
              {/* Bagian Atas - Jadwal */}
              <div>
                {onEdit ? <button className="px-3 py-1 shadow-md bg-blue-6 00 rounded-md text-sm relative right-0 hover:bg-blue-400 text-white font-bold" onClick={(e) => { setOnEdit(!onEdit) }}>konfirmasi</button> : onAdd ? "" : <button className="px-3 py-1 shadow-md bg-blue-600 rounded-md text-sm relative right-0 hover:bg-blue-400 text-white font-bold" onClick={(e) => { setOnEdit(!onEdit) }}>edit</button>
                }
                  ...
                  {jadwal.map((item, idx) => (
                    <div key={idx} className={`grid ${onEdit ? 'grid-cols-6' : 'grid-cols-5'} justify-items-center items-center mb-2`}>
                      ...
                      <div className="text-sm">{onEdit ? <div className="rounded"><select
                        value={jadwal[idx].status}
                        onChange={(e) => {
                          jadwal[idx].status = e.target.value;
                          updateJadwal(item._id, e.target.value)
                        }}
                        className="block w-full px-4 py-2 mt-1 text-sm border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                      >
                      ...
                      {
                        idx >= 3 && onEdit ? <div><button className="px-2 py-1 shadow-md bg-red-600 rounded-md text-sm relative right-0 hover:bg-red-400 text-white ml-4" onClick={(e) => { onDelete(item._id) }}>Hapus</button></div> : ""
                      }
                    </div>
                  ))}
                </div>
              ...
              <div className="border shadow-md rounded-md mb-10">
                <div className="text-center bg-koreaBlue rounded-t-md text-white font-semibold border-b border-black my-auto h-14 text-xl flex justify-center items-center">
                  <p className="my-auto">LIST PESERTA</p>
                </div>
                <div className="p-5">
                  {listJadwal.map((v, idx) => (
                    <div key={idx} onClick={(e)=>{window.location.href = "/lgndmn/dashboard/pendaftaran/"+`${v.hari}_${v.tanggal}_${v.bulan}_${v.tahun}`}} className="shadow-md hover:bg-gray-200 border mx-2 my-2 px-4 py-3 rounded cursor-pointer">
                      {v.hari} {v.tanggal} {v.bulan} {v.tahun}  
                    </div>
                  ))}
                </div>
