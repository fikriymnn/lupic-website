...
const BooksModulesPage = () => {
  const [books, setBooks] = useState([]);

  useEffect(() => {
    async function getData() {
      try {
        const response = await axios.get(process.env.NEXT_PUBLIC_API_URL + "/api/modul");
        if (response.data) {
    ...
    <>
      ...
      <div className="container mx-auto py-16">
        <div className="max-w-6xl px-4 md:px-8 text-center m-auto mb-8">
                <h3 className="md:text-5xl text-3xl md:mt-8 mt-4 font-bold">Books/Modules</h3>
...
export default BooksModulesPage;
